...
/** Supersample canvas — tăng lên 3 nếu cần nét hơn nữa */
const TEX_SCALE = 2;
...
/** Dịch cả khối trụ + ribbon xuống (world Y, âm = thấp hơn) */
const GROUP_Y_OFFSET = -0.4;
/** Nghiêng khối trụ sang phải (rad, âm = top lean +X) */
...
function RibbonCanvas({ ribbonText }: { ribbonText: string }) {
  ...
  useGSAP(
    () => {
      ...
      const tweens = scene.textures.map(({ texture, direction, duration }) => {
    ...
    { scope: containerRef, dependencies: [ribbonText] },
  );
    ...
    <header className="relative isolate w-full overflow-hidden bg-[var(--color-canvas)]">
      <div className="pointer-events-none absolute left-1/2 top-0 z-0 h-[min(48vw,380px)] min-h-[260px] w-screen -translate-x-1/2">
        <div className="h-full w-[125vw] translate-x-[2vw] scale-[1.05] origin-[84%_45%]">
          <RibbonCanvas ribbonText={ribbonText} />
        </div>
      </div>
